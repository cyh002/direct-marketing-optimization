...
for product in products:
    ...
    if os.path.exists(model_dir):
        ...
        if meta:
            ...
            with model_col2:
                st.metric("Train Score", f"{train_score:.4f}" if isinstance(train_score, (int, float)) else "N/A")
            with model_col3:
                st.metric("Test Score", f"{test_score:.4f}" if isinstance(test_score, (int, float)) else "N/A")
