...
def load_and_process_data(run_dir):
    ...
    detailed_results = offers.merge(
        prop[['Client'] + probability_columns], 
        on='Client', 
        how='left'
    ).merge(
    ...
    for product in products:
        ...
        detailed_results.loc[mask, 'purchase_probability'] = detailed_results.loc[mask, f'probability_{product}']
        detailed_results.loc[mask, 'expected_revenue'] = detailed_results.loc[mask, f'expected_revenue_{product}']
...
with col4:
    roi = results.get('roi', (results['total_revenue'] / (len(offers) * cost_per_contact)) - 1)
    st.metric(
        "ROI", 
        f"{roi:.2f}x",
...
offers_with_prob = offers.merge(
    prop, on='Client', how='left'
).merge(
...
for product in products:
    ...
    if count > 0:
        ...
        rev_per_contact = offers_with_prob.loc[mask, f'expected_revenue_{product}'].mean()
...
if not product_revenue_df.empty:
    ...
    display_df['Revenue per Sale'] = display_df['Revenue per Sale'].apply(lambda x: f"€{x:.2f}")
    display_df['Expected Revenue'] = display_df['Expected Revenue'].apply(lambda x: f"€{x:.2f}")
...
try:
    ...
    if os.path.exists(test_path):
        ...
        if 'Client' in test_df.columns:
            ...
            demo_cols = [c for c in merged_data.columns if c in ['Age', 'Sex', 'Sex_F', 'Sex_M']]
            
            if demo_cols:
                st.write("Demographic breakdown of targeted customers:")
                
                tabs = st.tabs(["Age Distribution", "Gender Distribution", "Product by Age"])
                ...
                with tabs[1]:
                    ...
                    if 'Sex' in demo_cols:
                        ...
                        merged_data['Gender'] = merged_data.apply(
                            lambda row: 'Female' if row.get('Sex_F', 0) == 1 else 'Male', 
                            axis=1
                        )
                        gender_col = 'Gender'
                    
                    if gender_col:
                        gender_counts = merged_data[gender_col].value_counts().reset_index()
...
try:
    ...
    if os.path.exists(eval_path):
        ...
        if not historical.empty:
            ...
            comparison_data = {
                'Metric': ['Total Revenue', 'Revenue per Contact', 'Acceptance Rate', 'ROI'],
                'Current': [
            ...
            formatted_df['Current'] = formatted_df.apply(
                lambda row: f"€{row['Current']:,.2f}" if 'Revenue' in row['Metric'] 
                else f"{row['Current']*100:.2f}%" if row['Metric'] == 'Acceptance Rate'
                else f"{row['Current']:.2f}x",
                axis=1
            )
            formatted_df['Historical Avg'] = formatted_df.apply(
                lambda row: f"€{row['Historical Avg']:,.2f}" if 'Revenue' in row['Metric'] 
                else f"{row['Historical Avg']*100:.2f}%" if row['Metric'] == 'Acceptance Rate'
            ...
            for i, metric in enumerate(comparison_df['Metric']):
                fig.add_trace(go.Bar(
                    ...
                    y=[comparison_df.loc[i, 'Current'], comparison_df.loc[i, 'Historical Avg']],
                    name=metric
                ))
...
for idx, row in detailed_results.iterrows():
    ...
    if product in products:
        detailed_results.loc[idx, 'purchase_probability'] = row[f'probability_{product}']
        detailed_results.loc[idx, 'expected_revenue'] = row[f'expected_revenue_{product}']
