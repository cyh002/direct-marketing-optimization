...
if prop is not None and rev is not None:
    ...
    for idx, row in merged_offers.iterrows():
        ...
        if product in products:
            merged_offers.loc[idx, 'purchase_probability'] = row.get(f'probability_{product}', None)
            merged_offers.loc[idx, 'expected_revenue'] = row.get(f'expected_revenue_{product}', None)
else:
...
if 'expected_revenue' in merged_offers.columns:
    rev_threshold = st.sidebar.slider(
        ...
        min_value=0.0, 
        max_value=float(merged_offers['expected_revenue'].max()) if not merged_offers['expected_revenue'].empty else 100.0,
        value=0.0,
        step=10.0,
        format="%.2f"
    )
...
if prob_threshold > 0 and 'purchase_probability' in filtered_offers.columns:
    filtered_offers = filtered_offers[filtered_offers['purchase_probability'] >= prob_threshold]
    
if rev_threshold > 0 and 'expected_revenue' in filtered_offers.columns:
    filtered_offers = filtered_offers[filtered_offers['expected_revenue'] >= rev_threshold]
    
if search_term:
    filtered_offers = filtered_offers[filtered_offers['Client'].astype(str).str.contains(search_term)]
...
st.write(f"Showing {len(filtered_offers)} clients (filtered from {len(merged_offers)} total)")

# Add formatting
if 'purchase_probability' in filtered_offers.columns:
    filtered_offers['purchase_probability'] = filtered_offers['purchase_probability'].apply(
...
with col2:
    page_size = max_rows
    total_pages = (len(filtered_offers) - 1) // page_size + 1 if len(filtered_offers) > 0 else 1
