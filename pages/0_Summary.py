...
if summary_df.empty:
    st.info("No run directories found in 'outputs'. Please run the optimization workflow first.")
    st.stop()
...
with tab1:
    ...
    score_df["score_type"] = score_df["score_type"].map({
        "best_train_score": "Train Score",
        "best_test_score": "Test Score"
    })
...
with tab2:
    ...
    summary_df["avg_rank"] = summary_df[["total_revenue", "mean_probability", 
                                        "best_train_score", "best_test_score"]].rank(ascending=False).mean(axis=1)
