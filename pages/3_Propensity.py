...
for product in products:
    ...
    if test_df is not None and f"Sale_{product}" in test_df:
        ...
        hist_data = [y_prob[y_true == 0], y_prob[y_true == 1]]
        ...
        if meta:
            ...
            with model_col2:
                st.metric(
                    "Train Score",
                    (
                        f"{meta.get('train_score', 'N/A'):.4f}"
                        if isinstance(meta.get("train_score"), (int, float))
                        else "N/A"
                    ),
                )
            with model_col3:
                st.metric(
                    "Test Score",
                    (
                        f"{meta.get('test_score', 'N/A'):.4f}"
                        if isinstance(meta.get("test_score"), (int, float))
                        else "N/A"
                    ),
                )
